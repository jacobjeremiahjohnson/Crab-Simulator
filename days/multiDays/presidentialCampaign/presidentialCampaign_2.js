...
export async function execute(queue) {
	...
	if(answer == 1) {
		...
		await fprint("You whip out your phone and type a real quick message informing them of your decision to campaign.\n", "dim", 1 )
		config.playAudio("./audio/ping.mp3")

		const span = config.createSpan("white")
		span.classList.add("choice")
		let discordMessage = config.createSpan("white")
		discordMessage.innerHTML = "[CrabbyMan#8644] : @everyone, Im running for president so yall should vote for me thanks"
		span.appendChild(discordMessage)
		config.output.appendChild(span)
		await config.sleep(5)

		config.playAudio("./audio/ping.mp3")
		discordMessage = config.createSpan("white")
		discordMessage.innerHTML = "[OldBayStan#5277] : ok buddy"
		span.appendChild(discordMessage)
		await config.sleep(2)

		config.playAudio("./audio/ping.mp3")
		discordMessage = config.createSpan("white")
		discordMessage.innerHTML = "[JohnnyFisher#2012] : aight"
		span.appendChild(discordMessage)
		fprint("", "white", 2, 0)
